...
class Menu_kabupaten extends BaseController
{
    ...
    public function inputdatakawasan()
    {
        if (isset($_POST['submit'])) {
            ...
            $this->validation->setRule('image5', 'Image5', 'trim|uploaded|mime_in[image5,image/png,image/jpg,image/jpeg]|max_size[image5,4096]', ['uploaded' => 'Gambar peta Deliniasi harus diisi', 'mime_in' => 'Ekstensi peta Deliniasi harus png/jpg/jpeg', 'max_size' => 'Maximal file size peta Deliniasi harus <= 4mb']);
            if (!$this->validation->withRequest($this->request)->run()) {
                return redirect()->to('user/menu-kabupaten/input_data_kawasan')->withInput();
            } else {
